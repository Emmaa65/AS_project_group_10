...
private:
    ...
    size_t findBestCluster(const std::vector<pcl::PointIndices> &clusters,
        ...
        // ── 3. Gewichte ─────────────────────────────────────────────────────────
        // Summe = 1.0  →  Score ∈ [0, 1]
        const double W_Z_DEPTH     = 0.05;  // Vertikale Tiefe
        const double W_DISTANCE    = 0.60;  // Nähe (invertiert)
        const double W_CONTINUITY  = 0.10;  // Richtungstreue
        ...
        for (auto &f : feats) {
            ...
            RCLCPP_INFO(get_logger(),
                "Cluster %zu | z=%.2f dist=%.2f cont=%.2f entr=%.2f → score=%.4f",
                f.idx, s_z_depth, s_distance, s_continuity, s_entrance, score);
